...
def start_test(test_id, student_id):
    try:
        ...
        if not success:
            return success, payload, status_key
        ...
        if session:
            status, content = _check_session_submitted(
                session,
                test_id,
                student_id,
                payload
        ...
        question_order = prepare_questions(payload['questions'])

        started_at = datetime.now(timezone.utc)
        expire_date = started_at + timedelta(minutes=payload['time_limit'])
...
def submit_test(test_id, student_id, answers):
    try:
        ...
        answers, score = _verify_answers(answers, question_map)
        ...
        completed_questions = _get_completed_questions(
...
def prepare_questions(question_list):
    ...
    for question in question_list:
        answers = shuffle_answers(question.get("answers", []))
...
def _verify_answers(answers, question_map):
    score = 0
...
def _get_completed_questions(answers, question_map):
    completed_questions = []
